...
class Visualisation(ABC):
    ...
    def _save_fig(self, file_extension: Optional[str] = 'pdf', **kwargs) -> None:
        """Save plot."""
        if self.save:
...
class VisRealDT(Visualisation):
    """
    Plots (normalized) death toll since first day when data were collected up to `last_day_do_plot`.

    Death tolls for all voivodeships can be normalized to one to show when pandemic started
...
class VisRealDTShiftedByHand(Visualisation):
   ...
   """
    ...
    def _add_simulated_data_to_plot(self, common_deaths: int) -> None:
        ...
        def get_beta_mortality_visibility(_fname: str) -> (float, float, float):
            _beta = float(variable_params_from_fname(fname=fname)['beta'])
            _mortality = float(variable_params_from_fname(fname=fname)['mortality'])
            _visibility = float(variable_params_from_fname(fname=fname)['visibility'])
        ...
        def find_common_day_by_shifting_death_toll(_death_toll: np.ndarray) -> int:
            try:
                return np.where(_death_toll >= common_deaths)[0][0]
            except IndexError:
                return 0

        def get_label(_beta: float, _mortality: float, _visibility: float) -> str:
        ...
        def get_hand_shift_num_of_days(_voivodeship: str) -> int:
            ...
            for deaths, voivodeships in death_shifts.items():
                if _voivodeship in voivodeships:
                    shift_date = RealData.get_date_of_first_n_death(deaths)[voivodeship]
                    return RealData.date_to_day(shift_date)
            raise KeyError

        hand_shift_num_of_days = get_hand_shift_num_of_days(voivodeship)
        return RealData.day_to_date(day_number=hand_shift_num_of_days + self.starting_day)
    ...
    def _make_inner_plot(self,
        ...
        for voivodeship, color in zip(voivodeships, self._get_colors(voivodeships)):
            ...
            self.ax.plot(x[:self.num_of_days_to_plot], y[:self.num_of_days_to_plot],
...
class VisBestTunedDTDynamic(Visualisation):
    ...
    @staticmethod
    def _get_day_sim_values_dict(simulated_toll: np.ndarray, shift: int) -> dict[int: float]:
    ...
    def _plot_and_annotate_data(self,
        ...
        day_real_recovered_toll = self._get_day_sim_values_dict(real_recovered_toll, 0)
        day_simulated_death_toll = self._get_day_sim_values_dict(simulated_death_toll, shift)
        day_simulated_recovered_toll = self._get_day_sim_values_dict(simulated_recovered_toll, shift)

        self.ax1.plot(
            [item[0] for item in day_real_death_toll.items() if item[0] <= self.last_day_to_plot],
            [item[1] for item in day_real_death_toll.items() if item[0] <= self.last_day_to_plot],
            label='raportowana suma zgonów', c=self.color_real, lw=2, zorder=1)

        self.ax1.plot(
            [item[0] for item in day_simulated_death_toll.items() if item[0] <= self.last_day_to_plot],
            [item[1] for item in day_simulated_death_toll.items() if item[0] <= self.last_day_to_plot],
            label='symulowana suma zgonów', c=self.color_simulated, lw=5, alpha=0.6, zorder=1)

        self.ax2.plot(
            [item[0] for item in day_real_recovered_toll.items() if item[0] <= self.last_day_to_plot],
            [item[1] for item in day_real_recovered_toll.items() if item[0] <= self.last_day_to_plot],
            label='raportowana suma zgonów', c=self.color_real, lw=2, zorder=1)

        self.ax2.plot(
            [item[0] for item in day_simulated_recovered_toll.items() if item[0] <= self.last_day_to_plot],
            [item[1] for item in day_simulated_recovered_toll.items() if item[0] <= self.last_day_to_plot],
            label='symulowana suma zgonów', c=self.color_simulated, lw=5, alpha=0.6, zorder=1)
    ...
    def _make_basic_plot(self,
        ...
        self._plot_and_annotate_data(real_death_toll=_real_death_toll,
                                     ...
                                     simulated_death_toll=sim_avg_df['Dead people'],
                                     simulated_recovered_toll=sim_avg_df['Recovery people'],
                                     shift=int(_df_best_run_details['shift']),
                                     num_of_first_day=int(_df_best_run_details['first day']),
                                     num_of_last_day=int(_df_best_run_details['last day']),
                                     )

        self._label_plot_components(voivodeship=_df_best_run_details['voivodeship'],
    ...
    def _make_plot_nice_looking(self, day_real_death_toll: dict[int, float]) -> None:
        """
        Annotate day 0 as 2020-03-04.
        ...
        def improve_ax1_axis():
            ...
            self.ax1.set_xticklabels(
                [f'{x:.0f}' if x != 0 else '2020-03-04' for x in self.ax1.get_xticks().tolist()])

            max_plotted_death_toll_real = day_real_death_toll[self.last_day_to_plot]
            if max_plotted_death_toll_real != 0:
                self.ax1.set_ylim([-0.1 * max_plotted_death_toll_real, 1.5 * max_plotted_death_toll_real])
        ...
        def improve_ax2_axis():
            ...
            self.ax2.set_xticklabels(
                [f'{x:.0f}' if x != 0 else '2020-03-04' for x in self.ax2.get_xticks().tolist()])
    ...
    def _make_annotations(self,
        ...
        def annotate_ax1() -> None:
            ...
            self.ax1.axvline(num_of_last_day, c='red', label='dni do których dopasowywano model')

            max_death_toll_real = day_real_death_toll[self.last_day_to_plot]
            print(f"{self.last_day_to_plot = }")
            print(f"{max_death_toll_real = }")
            ...
            self.ax1.annotate(
                ...
                xy=((num_of_last_day + num_of_first_day) / 2, max_death_toll_real * 1.02),
                ha='center', va='bottom', fontsize=20,
                bbox=dict(boxstyle="square,pad=0",
                          fc=mColors.to_rgba('white')[:-1] + (0.8,),  # set `alpha=0.8`
                          ec="none"),
            )
        ...
        def annotate_ax2() -> None:
            ...
            sim_max_recovered_toll_plotted = day_simulated_recovered_toll[self.last_day_to_plot]
            y_annotate_offset = .1 * sim_max_recovered_toll_plotted

            # Annotate real_recovered_toll in last day of fit (with arrow and value)
            self.ax2.annotate(f"{day_real_recovered_toll[num_of_last_day] / 1000:.1f}k",
                              xy=(num_of_last_day, day_real_recovered_toll[num_of_last_day]),
                              xytext=(num_of_last_day - 10,
                                      day_real_recovered_toll[num_of_last_day] + .5 * y_annotate_offset),
                              c=self.color_real, fontsize=14, arrowprops=dict(arrowstyle="->"),
            ...
            self.ax2.annotate(f"{day_simulated_recovered_toll[num_of_last_day] / 1000:.1f}k",
                              xy=(num_of_last_day, day_simulated_recovered_toll[num_of_last_day]),
                              xytext=(num_of_last_day - 10,
                                      day_simulated_recovered_toll[num_of_last_day] + 2 * y_annotate_offset),
                              c=self.color_simulated, fontsize=14, arrowprops=dict(arrowstyle="->"),
                              bbox=dict(boxstyle="square,pad=0", fc="white", ec="none", lw=2))
    ...
    def plot(self):
        ...
        df_best_tuned_csv_details = OptimizeResultsWriterReader.get_n_best_tuned_results(n=1)
        ...
        for _, df_best_run_details in df_best_tuned_csv_details.iterrows():
            ...
            self._make_basic_plot(
                ...
                _real_death_toll=np.array(df_real_death_tolls.loc[voivodeship]),
                _real_recovered_toll=np.array(df_real_recovered_tolls.loc[voivodeship])
            )
...
class RealVisualisation:
    ...
    def show_real_death_toll(cls,
        ...
        Death tolls for all voivodeships is normalized to one to show when pandemic started
        for each voivodeship. Line color gradient corresponds to urbanization (red = biggest value)
        ...
        Parameters:
            ...
            :type show: bool
        """
        # warnings.warn("Please use `VisRealDT.plot()` instead of this.",
        #               DeprecationWarning)
        ...
        if last_day:
            last_day = min(last_day, len(RealData.death_tolls().columns))
        else:
            last_day = len(RealData.death_tolls().columns)
        ...
        if 'all' in voivodeships:
            num_of_lines = len(RealData.voivodeships())
        else:
    ...
    def show_real_death_toll_shifted_by_hand(cls,
        ...
        warnings.warn("Please use `VisRealDTShiftedByHand.plot()` instead of this.",
                      DeprecationWarning)
        ...
        for minimum_deaths, voivodeships in death_shifts.items():
            ...
            true_start_day = RealData.get_date_of_first_n_death(n=minimum_deaths)
    ...
    def plot_last_day_finding_process(cls,
        ...
        for voivodeship in voivodeships:
            ...
            # last of pandemic as day of peak closest to 2 moths with derivative > 0.5
    ...
    def plot_pandemic_time(cls,
        ...
        cls.__show_and_save(fig=fig,
                            ...
                            plot_name=(f'Pandemic time based on {starting_days_by.name}, '
    ...
    def compare_pandemic_time_by_infections_and_deaths(cls,
        ...
        ax = fig.add_subplot(111)
        # make ax title and axis labels
        main_info = ("Summary of designated positions in time of the first stage of the pandemic.\n"
...
class SimulatedVisualisation:
    ...
    def plot_stochastic_1D_death_toll_dynamic(cls,
        ...
        variable_params = variable_params_from_fname(fname=latest_file_in_dir(avg_directory))
    ...
    def death_toll_for_best_tuned_params(cls,
        ...
        def plot_matched_real_death_toll_to_simulated(
            ...
            last_day_to_search = list(RealData.death_tolls().columns).index(last_date_str)
        ...
        def make_plot(sub_df):
            ...
            def label_ax(ax: plt.axes, title: str, xaxis_label: str, yaxis_label) -> None:
                ax.set_title(title)
    ...
    def plot_best_beta_mortality_pairs(cls,
        ...
        fig.suptitle("Najlepiej dopasowane wartości "r'$\beta$'" i śmiertelności"
...
def main():
    ...
    # SimulatedVisualisation.death_toll_for_best_tuned_params(show=True, save=False)
