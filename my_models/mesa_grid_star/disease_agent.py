...
class OrdinaryPearsonAgent(Agent):
        ...
        if self.model.extra_shopping_boolean:
            if self.on_extra_shopping:
                ...
                neighbouring_neighbourhood_id =\
                    self.model.order_of_shopping_in_neighbouring_neighbourhoods[self.neighbourhood_id][
                        self.number_in_household][self.model.current_week]
    ...
    def try_to_infect_housemates(self):
        ...
        if housemates_to_infect:
            for housemate in housemates_to_infect:
                housemate.go_into_incubation_phase()
    ...
    def step(self):
        ...
        # Case in which pearson can die only at the end of illness period
        elif self.state == 3:
        ...
        # #Case in which pearson can die in each day during its illness period
        # elif self.state == 3:
        #     self.illness_period -= 1
...
class CashierAgent(Agent):
    ...
    def replace_cashier_if_needed(self):
        if self.state == 2 and self.prodromal_period <= 0:
