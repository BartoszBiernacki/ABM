...
from disease_spread_model.mesa_modified.mesa_time_modified import OrderedActivation
...
from .my_model_utils import create_array_of_extra_shopping_days_for_each_household_for_each_week
...
class DiseaseModel(Model):
    def __init__(self,
        ...
        self.total_households = int(self.num_of_households_in_neighbourhood * self.total_neighbourhoods)
        self.number_of_weeks_to_make_shopping_everywhere = nearest_neighbours(y=self.height, x=self.width)
        self.max_weeks = self.max_steps // 7 + 1
        self.cashiers_by_neigh = np.empty(self.total_neighbourhoods, dtype=CashierAgent)
        ...
        self.neigh_id_of_infected_cashiers_at_start =\
            random.sample(range(self.total_neighbourhoods), self.infected_cashiers_at_start)
        # *************************************************************************************************************
        ...
        self.neighbourhood_yx_position_to_id = np.empty((self.height, self.width), dtype=np.int8)
        #  neighbourhood_id --> [y, x]
        self.neighbourhood_id_to_yx_position = np.empty((self.total_neighbourhoods, 2), dtype=np.int8)

        # [neighbourhood_id] --> 0 or 1 or 2
        self.C_state_by_neigh_id = np.empty(self.total_neighbourhoods, dtype=np.int8)
        self.C_incubation_duration_by_neigh_id = np.zeros(self.total_neighbourhoods, dtype=np.int8)
        self.C_prodromal_duration_by_neigh_id = np.zeros(self.total_neighbourhoods, dtype=np.int8)
        # ************************************************************************************************************
        ...
        # Agent (extra) shopping T/F and is agent was infected on today's (extra) shopping ****************************
        ...
        if self.extra_shopping_boolean:
            if self.number_of_weeks_to_make_shopping_everywhere:
                ...
                array_to_fill = np.ones((self.max_weeks, self.total_households), dtype=np.int8)*(-1)
